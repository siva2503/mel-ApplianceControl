...
def buildClusterList(endpoint):
    ...
    """
    myList = []
    for clust in endpoint['servers']:
        myList.append(('server',clust))
    for clust in endpoint['clients']:
...
""" Thermostat Methods """
def setWeeklySchedule(myNodeId,myEp,myCluster,myDayBitmap,myPayload):
...
def imageNotify(myNodeId, myEp, mySendMode, myPayloadType, myQueryJitter, myManufCode, myImageType, myFileVersion,timeout=130):
    """ Send image notify message and wait for QueryNextImage from device
    
    """
    expectedResponse = ['IMGQUERY:{}'.format(myNodeId)]
    ...
    while time.time() < startTime+timeout: 
        ...
        print('Send command imageNotify has failed.',respValue)
        seqNum = None 
    ...
    return respState,respCode,respValue,seqNum
def queryNextImageResponse(myNodeId, myEp, mySendMode, myStatus, myManufCode, myImgType, myFileVersion,myImgSize, mySeq):
    """ Send QueryNextImage Response and wait for first BlockRequest
    
    """
    expectedResponse = ['IMGBREQ:{0}'.format(myNodeId),
    ...
    print()
    print(respValue)
    if respState and respCode==zcl.statusCodes['SUCCESS'] and respValue[1].startswith('UPGRADEREQ'):
        # Unexpected upgrade request
        print('UNEXPECTED UPGRADE END')
        exit()
    ########################## DEBUG
    ...
    if respState:
        ...
        print("QNI Response failed. ", respValue)
        seqNum = None
        offset = None
        maxDataSize = None
        
    return respState, respCode, respValue, seqNum, offset, maxDataSize
def imageBlockResponse(myNodeId,myEp,mySendMode,myStatus,myManufCode,myImageType,myFileVersion,myFileOffset,myDataSize,
                       myPayload,mySeq):
    """ Send a block and wait for reply
    
    """
    # Response is either sequence number for next block or an endUpgradeRequest
    # If reply is endUpgrade then send endUpgradeResponse and exit else return with a seq number
    ...
    # IMGBREQ:<NodeID>,<EP>,<FiledControl>, <ManufCode>,<ImgType>,<FileVer>, <Offset>,<MaxDataSize> [,<RequestNodeAddress>, <BlockRequestDelay>],<SequenceNumber>
    # UPGRADEREQ: <NodeID>,<EP>,<Status>, <ManufCode>,<ImgType>,<FileVer>, <SequenceNumber>
    
    if respState:
    
        # If response is imageBlockRequest 
        if respValue.startswith(expectedResponse[0]):
            resp = respValue.split(',')
            seqNum = resp[8]
            offset = resp[6]
            done = False
        # Else if it's an updateRequest
        elif respValue.startswith(expectedResponse[1]):
            ...
            respState,respCode, respValue = sendCommand(msg, expectedResponse)
            if respState:
                print('Image upgrade sent - device should now reboot')
                done = True
                seqNum = None
                offset = None
            else:
                print('ERROR: Image upgrade sending has failed.', respValue)
    else:
        print('ERROR: imageBlockResponse sending has failed. ',respValue)
        seqNum = None
        offset = None
        done = False
        
    return respState, respCode, respValue, seqNum, offset, done
def waitForQNI(myNodeId):
    while True:
        while not rxQueue.empty():
            resp = rxQueue.get()
            if resp.startswith('IMGQUERY:{0}'.format(myNodeId)):
                print(resp)
                seq = resp.split(',')[6]
                print('send QNI response..')
                cmd = 'AT+QIMGRSP:{0},09,0,98,{1}'.format(myNodeId, seq)
                sendCommand(cmd,['OK'])
                exit()
    return 0

""" Abstracted Methods """
def getInitialData(nodeId, fastPoll=True, printStatus=False):
                ...
                for attr in respValue:
                    ...
                    if respState and respCode==zcl.statusCodes['SUCCESS']:
                        if respVal=='86':
                            ...
                            respStatus,respCode,attrReport = getAttributeReporting(myNodeId, ep, clustId, clustType, attrId)
                            if not respStatus:
...
if __name__ == "__main__":
    ...
    nodeId, port = readArguments()
    
    if nodeId=='': nodeId=lc.node1
